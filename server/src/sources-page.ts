...
export interface SpatialAssetSummary {
  id: string;
  name: string;
  assetClass: string;
...
export function renderSourcesPageHtml(sources: SourceSummary[], assets: SpatialAssetSummary[] = []): string {
  ...
  const assetCards = assets.map((asset) => {
    const geography = [asset.adminLevel1, asset.adminLevel2, asset.locality].filter(Boolean).join(" · ") || "Location pending";
    const searchable = [asset.name, asset.assetClass, asset.authorityName, geography].filter(Boolean).join(" ").toLowerCase();
    return `<article class="source-card" data-availability="asset" data-search="${esc(searchable)}">
      <div class="source-head"><span class="type">${esc(asset.assetClass.replace(/_/g, " "))}</span><span class="status">${esc(asset.processingStatus)}</span></div>
      <h2>${esc(asset.name)}</h2><p class="provider">${esc(asset.authorityName ?? "Authority pending")}</p>
      <dl><dt>Location</dt><dd>${esc(geography)}</dd><dt>Boundary</dt><dd>${esc(asset.geometryStatus.replace(/_/g, " "))}</dd><dt>Service use</dt><dd>${asset.checkStatus === "eligible" ? "Used in checks" : "Excluded from checks"}</dd></dl>
      ${asset.sourceUrl ? `<a href="${esc(asset.sourceUrl)}" rel="noreferrer">Discovery source →</a>` : ""}
    </article>`;
  }).join("");
